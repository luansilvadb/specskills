...
exports.AdvancedSkillRecommender = exports.AdvancedSkillScorer = void 0;
class AdvancedSkillScorer {
    constructor() {
        this.defaultWeights = {
            keywordMatch: 0.25,
     ...
     */
    calculateScores(skills, context, weights = {}) {
     ...
     */
    calculateFactors(skill, context) {
        return {
     ...
     */
    calculateKeywordMatch(skill, context) {
     ...
     */
    calculateSemanticSimilarity(skill, context) {
     ...
     */
    calculateContextualRelevance(skill, context) {
        let score = 0;
        ...
        if (context.techStackContext) {
            ...
            const techMatches = skill.techAffinity.filter(affinity => techContext.includes(affinity.toLowerCase())).length;
            score += (techMatches / skill.techAffinity.length) * 0.4;
        }
        // Prompt relevance
        if (context.trackPrompt) {
            const promptContext = context.trackPrompt.toLowerCase();
            const keywordMatches = skill.keywords.filter(keyword => promptContext.includes(keyword.toLowerCase())).length;
     ...
     */
    calculateHistoricalPerformance(_skill, context) {
     ...
     */
    calculateUserPreferenceAlignment(skill, context) {
     ...
     */
    calculateDependencyFit(skill, _context) {
     ...
     */
    calculateResourceEfficiency(skill, _context) {
     ...
     */
    aggregateScore(factors, weights) {
     ...
     */
    getIdentifyRelatedTerms(context1, context2) {
     ...
     */
    areSemanticallyRelated(token, candidates) {
        ...
        return candidates.some(candidate => this.areSimpleVariants(token, candidate) ||
            this.areSynonymLike(token, candidate));
    }
    /**
     * Check if tokens are simple variants (plural, tense, etc.)
     */
    areSimpleVariants(token1, token2) {
    ...
    areSynonymLike(token1, token2) {
        ...
        return synonymGroups.some(group => group.includes(token1) && group.includes(token2));
    }
    /**
     * Estimate skill complexity based on various factors
     */
    estimateSkillComplexity(skill) {
    ...
    estimateSkillEffort(skill) {
        ...
        const effortWords = effortIndicators.filter(indicator => skill.content.toLowerCase().includes(indicator) ||
...
exports.AdvancedSkillScorer = AdvancedSkillScorer;
/**
 * Utility class to recommend skills using the advanced scorer
 */
class AdvancedSkillRecommender {
    constructor() {
     ...
     */
    recommendSkills(skills, context, minScore = 0.3, maxResults = 10) {
        // Calculate scores
     ...
     */
    createContext(trackDescription, productContext = '', techStackContext = '') {
        return {
...
exports.AdvancedSkillRecommender = AdvancedSkillRecommender;

...
exports.newTrackCommand = {
    ...
    execute: async (context) => {
        try {
            ...
            const recommender = new skillComposition_1.CompositeSkillRecommender(compositionEngine);
            ...
            if (scoredSkills.length > 1) {
                const composition = compositionEngine.createCompositionFromSkills(`auto-${trackId}`, `Track ${trackId} Skills Composition`, `Automatically generated composition for track ${trackDescription}`, scoredSkills.map(s => s.skill), [trackId, ...trackDescription.split(/\s+/)]);
            ...
            const existingSkills = recommendedSkills.filter(skill => {
                ...
                return (0, fileSystem_1.fileExists)(skillRootFile) || (0, fileSystem_1.fileExists)(skillDirFile);
            });
            // 2. Create track directory
            (0, fileSystem_1.ensureDir)(trackDir);
            ...
            const specContent = (0, templates_1.processTemplate)(conductorDir, 'spec.md', templateVars, generateSpecTemplate(trackDescription, existingSkills, potentialCompositions));
            const planContent = (0, templates_1.processTemplate)(conductorDir, 'plan.md', templateVars, generatePlanTemplate(trackDescription, existingSkills, potentialCompositions, trackId));
            const indexContent = (0, templates_1.processTemplate)(conductorDir, 'track-index.md', templateVars, generateTrackIndex(trackDescription, trackId));
            ...
            (0, fileSystem_1.writeFile)(path.join(trackDir, 'spec.md'), specContent);
            (0, fileSystem_1.writeFile)(path.join(trackDir, 'plan.md'), planContent);
            (0, fileSystem_1.writeFile)(path.join(trackDir, 'index.md'), indexContent);
...
function generateMindsetSection(skills, compositions = []) {
...
function generateProtocolsSection(skills, compositions = []) {
    if (skills.length === 0 && compositions.length === 0)
        return '';
...
function generateSpecTemplate(description, skills, compositions = []) {
    let skillSections = '';
    if (skills.length > 0 || compositions.length > 0) {
        skillSections = '\n## Architecture Mindset\n\n';
        for (const skill of skills) {
...
function generatePlanTemplate(description, skills, compositions = [], trackId) {
...
function generateTrackIndex(description, trackId) {
...
async function updateTracksIndex(conductorDir, description, trackId) {
    const indexPath = path.join(conductorDir, 'index.md');
    let content = (0, fileSystem_1.readFile)(indexPath);
    if (!content) {
    ...
    (0, fileSystem_1.writeFile)(indexPath, content);
}

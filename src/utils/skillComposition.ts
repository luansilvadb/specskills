...
import type { Skill, SkillWithScore, SkillExecutionContext } from '../types';
...
export interface SkillComposition {
  ...
  description: string;
  skills: Skill[];
  dependencies: string[]; // IDs of skills this composition depends on
  executionOrder: string[]; // Order in which skills should be executed
  triggerConditions: string[]; // Conditions that trigger this composition
}
...
export class SkillCompositionEngine {
  ...
  findMatchingCompositions(triggerConditions: string[]): SkillComposition[] {
    return Array.from(this.compositions.values()).filter(comp =>
      comp.triggerConditions.some(condition =>
        triggerConditions.some(tc => tc.toLowerCase().includes(condition.toLowerCase()))
      )
    );
  }
   ...
   */
  private async executeSingleSkill(context: SkillExecutionContext): Promise<Record<string, any>> {
   ...
   */
  private applyDirectives(directives: string, context: Record<string, any>, _parameters: Record<string, any>): Record<string, any> {
   ...
   */
  private validateDependencies(composition: SkillComposition): string[] {
 ...
 */
export class CompositeSkillRecommender {
  private compositionEngine: SkillCompositionEngine;
   ...
   */
  recommendSkillsAndCompositions(
    contextDescription: string,
    allSkills: Skill[]
  ): { skills: SkillWithScore[]; compositions: SkillComposition[] } {
    ...
    const recommendedSkills: SkillWithScore[] = allSkills.map(skill => {
      let score = 0;
      ...
      if (skill.purpose && tokens.some(token => (skill.purpose as string).toLowerCase().includes(token))) {
        score += 8;
      }
   ...
   */
  private findPotentialCompositions(skills: Skill[]): SkillComposition[] {
    // Identify common domains and create compositions for related skills
    const domainGroups = new Map<string, Skill[]>();
    ...
    const compositions: SkillComposition[] = [];
    ...
    for (const [domain, domainSkills] of domainGroups) {
      if (domainSkills.length > 1) {
        const composition = this.compositionEngine.createCompositionFromSkills(
          `comp-${domain.replace(/\s+/g, '-')}`,
          `${domain} Skills Composition`,
          `Composition of related ${domain} skills`,
          domainSkills,
          [domain, ...domainSkills.map(s => s.id)]
        );

...
export function parseSpecDocument(content: string): ParsedSpecDocument {
        ...
        switch (key.toLowerCase()) {
          ...
          case 'status':
            ...
            if (['draft', 'review', 'approved', 'deprecated'].includes(statusValue)) {
              spec.status = statusValue as 'draft' | 'review' | 'approved' | 'deprecated';
            }
            break;
        }
      }
  ...
  const reqLines = content.split('\n').filter(line =>
    line.trim().match(/^-\s+\*\*Requirement\*\*:|^-\s+REQ-\d+:|^- ID:/i)
  );
 ...
 */
export function parseTracksIndex(content: string): ParsedTrack[] {
 ...
 */
function parseTracksWithLegacyFormat(content: string): ParsedTrack[] {
 ...
 */
function getStatusFromChar(statusChar: string): 'pending' | 'in_progress' | 'completed' {
 ...
 */
function getStatusFromText(statusText: string): 'pending' | 'in_progress' | 'completed' {
 ...
 */
function parseTracksWithSeparators(content: string): ParsedTrack[] {
  const tracks: ParsedTrack[] = [];
    ...
    if (headerLine) {
      ...
      if (match) {
        ...
        if (linkLine) {
          ...
          if (lMatch) folderPath = lMatch[1].trim();
        }
...
export function updateTaskStatus(
  ...
  const pattern = new RegExp(
    `(-\\s*\\[)(?:\\s|~|x)(\\]\\s*${escapedDescription})(?:\\s*\\[commit:\\s*[a-f0-9]+\\])?$`,
    'i'
  );
 ...
 */
export function countTasksByStatus(tasks: ParsedTask[]): {
  total: number;
  completed: number;
  inProgress: number;
  pending: number;
 ...
 */
export function reconcileCatalog(content: string, skills: any[]): string {

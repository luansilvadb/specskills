 ...
 */
export async function processInteractiveQuestion(question: InteractiveQuestion): Promise<UserResponse> {
 ...
 */
export async function processInteractiveQuestions(questions: InteractiveQuestion[]): Promise<UserResponse[]> {
 ...
 */
function getDefaultValueForQuestionType(type: string): string | string[] | boolean {
  switch (type) {
    case 'yesno':
      return false;
    case 'choice':
 ...
 */
export function validateUserResponse(question: InteractiveQuestion, response: any): { isValid: boolean; error?: string } {
  ...
  switch (question.type) {
    ...
    case 'choice':
      if (question.multiSelect) {
        ...
        if (question.options) {
          const invalidOptions = response.filter((ans: any) =>
            !question.options!.some(opt => opt.label === ans)
          );
 ...
 */
export function transformQuestionResult(question: InteractiveQuestion, answer: any): any {
  switch (question.type) {
    case 'yesno':
